...
export function useGeminiAnalysis() {
  ...
  const getAnalysis = async ({ prices, coinName, usdToIdrRate }) => {
    setIsAnalyzing(true);
    setError(null);
    setAnalysis(null);
    ...
    try {
      ...
      const prompt = `
        ...
        3.  **Perkiraan Aksi (dalam Rupiah)**: Berikan perkiraan titik masuk (area beli) dan titik keluar (area jual/take-profit) yang masuk akal. **Konversikan juga semua nilai harga ini ke dalam Rupiah (IDR).** Jelaskan alasan singkat di baliknya.
