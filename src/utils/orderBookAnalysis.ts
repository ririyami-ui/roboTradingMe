...
export interface OrderBookAnalysisResult {
    ...
    sellPressure: number;
    spread: number;
    status: 'NEUTRAL' | 'BULLISH_PRESSURE' | 'BEARISH_PRESSURE' | 'LOW_LIQUIDITY';
 ...
 * @param {any} depthData - Data dari Indodax fetchOrderBook
 * @param {number} rangePercent - Range harga dari mid-price untuk dianalisis (default 2%)
 * @returns {OrderBookAnalysisResult} Hasil analisis
 */
export const analyzeOrderBook = (depthData: any, rangePercent = 2.0): OrderBookAnalysisResult => {
    if (!depthData || !depthData.buy || !depthData.sell || depthData.buy.length === 0 || depthData.sell.length === 0) {
        return { isValid: false, imbalance: 0, buyPressure: 0, sellPressure: 0, spread: 0, status: 'NEUTRAL', totalBuyVolume: 0, totalSellVolume: 0 };
    }

    const bestBid = parseFloat(depthData.buy[0][0]);
    const bestAsk = parseFloat(depthData.sell[0][0]);
    
    // Validasi harga waras
    ...
    let status: 'NEUTRAL' | 'BULLISH_PRESSURE' | 'BEARISH_PRESSURE' | 'LOW_LIQUIDITY' = 'NEUTRAL';
 ...
 * @param {any[]} orders - Array orders [[price, amount], ...]
 * @param {number} thresholdFactor - Faktor pengali dari rata-rata order (default 5x)
 */
export const findMarketWalls = (orders: any[], thresholdFactor = 5.0): OrderBookEntry[] => {
