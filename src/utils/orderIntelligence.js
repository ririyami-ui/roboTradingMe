import { fetchOrderBook } from './indodaxApi';
 ...
 * @param {object} order - Objek order dari Indodax (openOrders)
 * @param {number} currentPrice - Harga pasar saat ini
 * @returns {object} { shouldCancel: boolean, reason: string }
 */
export const checkBuyOrderIntelligence = (order, currentPrice) => {
 ...
 * @param {object} order - Objek order dari Indodax
 * @param {object} depthData - Data order book terbaru
 * @returns {object} { shouldAdjust: boolean, newPrice: number, reason: string }
 */
export const checkSellOrderIntelligence = (order, depthData) => {
